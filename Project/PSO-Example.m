...
for i = 1 : 2
    for j = 1 : 4
        swarm(index, 1, 1) = randi(25,1);
        swarm(index, 1, 2) = randi(25,1);
        index = index + 1;
    end
end
...
for iter = 1 : iterations
    ...
    for i = 1 : swarm_size
        swarm(i, 2, 1) = rand*swarm(i, 2, 1) + correction_factor*rand*(swarm(i, 3, 1) - swarm(i, 1, 1)) + correction_factor*rand*(swarm(gbest, 3, 1) - swarm(i, 1, 1));   %x velocity component
        swarm(i, 2, 2) = rand*swarm(i, 2, 2) + correction_factor*rand*(swarm(i, 3, 2) - swarm(i, 1, 2)) + correction_factor*rand*(swarm(gbest, 3, 2) - swarm(i, 1, 2));   %y velocity component
    end
